...
def train_constraint_loss(model, ae_model, feature_dist, constrained_feat_indices, train_dataset, optimizer, validity_reg, constraint_reg, ae_reg, normalise_weights, margin, delta_case, epochs=1000, batch_size=1024):
    batch_num=0 
    train_loss=0.0
    train_size=0
    #train_dataset= np.array_split( train_dataset, train_dataset.shape[0]//batch_size ,axis=0 )
...
parser.add_argument('--constrained_nodes', type=str, default='..')
...
if dataset_name=='bn1':
    ...
    path= base_model_dir + 'sangiovese-512-50-auto-encoder.pth'
...
model_param=[]
constrained_nodes= args.constrained_nodes.split(',')
print(constrained_nodes)

#Training Linear Model for all the constraint nodes
