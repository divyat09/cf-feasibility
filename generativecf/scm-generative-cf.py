...
def scm_change_score(x, xpred, normalise_weights, delta_case):    
...
def train_scm_loss(model, train_dataset, optimizer, normalise_weights, validity_reg, scm_reg, margin, delta_case, epochs=1000, batch_size=1024):
    batch_num=0
    train_loss=0.0
    train_size=0
#     train_dataset= np.array_split( train_dataset, train_dataset.shape[0]//batch_size ,axis=0 )
...
for epoch in range(args.epoch):
    ...
    loss_val.append( train_scm_loss( cf_vae, vae_train_dataset, cf_vae_optimizer, normalise_weights, validity_reg, scm_reg, margin, delta_case, 1, batch_size) )
    print('Done training for epoch: ', epoch)
        
#Saving the final model
torch.save(cf_vae.state_dict(),  base_model_dir + dataset_name + '-delta-case-' + str(args.delta_case) + '-margin-' + str(args.margin) + '-scm_reg-'+ str(args.scm_reg) +  '-validity_reg-'+ str(args.validity_reg) + '-epoch-' + str(args.epoch) + '-' + 'scm-gen' + '.pth')
