...
def train_auto_encoder_loss(model, ae_model, train_dataset, optimizer, normalise_weights, validity_reg, ae_reg, margin, epochs=1000, batch_size=1024):
    batch_num=0
    train_loss=0.0
    train_size=0
#     train_dataset= np.array_split( train_dataset, train_dataset.shape[0]//batch_size ,axis=0 )
...
parser.add_argument('--ae_reg', type=float, default=0.5)
...
for epoch in range(args.epoch):
    ...
    loss_val.append( train_auto_encoder_loss( cf_vae, ae_vae, vae_train_dataset, cf_vae_optimizer, normalise_weights, validity_reg, ae_reg, margin, 1, batch_size) )

#Saving the final model
torch.save(cf_vae.state_dict(),  base_model_dir + dataset_name + '-margin-' + str(args.margin) + '-validity_reg-'+ str(args.validity_reg) + '-ae_reg-'+ str(args.ae_reg) +  '-epoch-' + str(args.epoch) + '-' + 'ae-gen' + '.pth')
