...
if dataset_name=='bn1':
    ...
    # 28*28 image default size of images in the case of MNIST
    inp_shape=28
    pred_model= BlackBox()
    learning_rate= 0.001
    #Default Batch Size of Keras
    batch_size= 32
    optimizer= optim.Adam([
        {'params': filter(lambda p: p.requires_grad, pred_model.predict_conv_net.parameters()) },
        {'params': filter(lambda p: p.requires_grad, pred_model.predict_fc_net.parameters()) }
    ], lr=learning_rate)
    crieterion= nn.CrossEntropyLoss()
...
for epoch in range(51):
    ...
    for i in range(len(train_batches)):    
        ...
        # Cross Entropy with L1 Regulariser
        loss= crieterion(out, train_y)
        # L2 regularizer on the weights of the model
        weight_norm = torch.tensor(0.)
        for w in pred_model.predict_fc_net.parameters():
        ...
        print('Train Loss for Batch: ', batch_num, loss)
    print('Done for Epoch: ', epoch , ' : Accuracy : ' , train_acc/len(train_dataset), len(train_dataset))
